...
#include "aAnalog.h"    /* analog routines */
...
#define aPORTNAME "tty.USA19QI1821P1.1"
...
int main(int argc, char* argv[]) {
  ...
  if (stubErr == aErrNone)
    aStem_SetStream(stemLib, linkStream, 
                    kStemModuleStream, &stubErr);
  ...
  cBytes[0] = 0x11;
  cBytes[1] = 0x23;
  stubErr = aServo_SetLimits(stemLib, aMODULE, aSERVO, (unsigned char*)cBytes);
  stubErr = aServo_GetLimits(stemLib, aMODULE, aSERVO, (unsigned char*)cBytes);
  ...
  for (i = 0; (i < 5) && (stubErr == aErrNone); i++) {
      ...
      stubErr = aServo_GetPosition(stemLib, aMODULE, aSERVO,
                                   &cValue);
  ...
  stubErr = aServo_SetConfig(stemLib, aMODULE, aSERVO, 
                             (unsigned char)(cValue | aSERVO_INV));      
  ...
  if (stubErr == aErrNone) {
    stubErr = aDigital_SetConfig(stemLib, aMODULE, 1, 
                                 aDIGITAL_OUTPUT);
